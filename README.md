...
The TCP and connected-UDP paths are implemented on IPv4 and IPv6 in both VLS
modes. HTTP/1.1, HTTP/2 (cleartext + TLS-with-ALPN), gRPC (unary +
server-streaming, cleartext + TLS), layered `crypto/tls`, context cancellation,
live I/O deadlines, Happy Eyeballs, concurrent I/O, shutdown, and VPP
configurations with multiple worker threads are covered. Mode 3 is currently
the only release-candidate path: the four-worker Mode 2 suite still exposes a
P0 concurrent-accept failure.
...
- `ListenPacket` implements a per-peer session adapter over VPP's
  ...
  connected UDP via `Dial("udp", addr)`. Both modes.
...
- Mode 2 UDP connects block on the worker thread until fully established to
  work around a VPP half-open session cleanup race. This is safe but means
  Mode 2 UDP connect latency is slightly higher than Mode 3's async path.
- Mode 2 listeners use per-worker sharding: one VLS listener per worker on the
  ...
  a shared channel. This preserves VLS session ownership, but concurrent accept
  load can currently crash the VCL application and poison its control ring.
  Do not deploy Mode 2 listeners until the
  [accept/MQ P0](docs/mode2_accept_mq_investigation.md) is fixed.
- The current CGo bridge links to `vls_unregister_vcl_worker`, supplied by the
  adjacent patched VPP review build. A stock `libvppcom.so` without that export
  cannot link this revision; upstream or explicitly carry and pin the patch.
...
| Connected UDP via `Dial("udp", "udp4", "udp6", ...)` | Integrated on IPv4 and IPv6 in both modes |
| Unconnected UDP via `ListenPacket` | Per-peer session adapter in both modes; `ReadFrom` for any peer, `WriteTo` to known peers only |
...
Connected UDP works in both Mode 3 and Mode 2.
...
1.26.1 and a patched VPP 26.10 development build. The patch exports
`vls_unregister_vcl_worker` for deterministic Mode 2 worker retirement; this
is a link-time requirement for the current source even when Mode 3 is selected.
...
Confirm that the chosen library contains the required teardown API:

```bash
nm -D /path/to/libvppcom.so | grep -w vls_unregister_vcl_worker
```

The local adjacent VPP review commit `032b24d04` supplies it. Until an upstream
release contains the API, deployments must carry and document an equivalent
patch.
...
Mode 2 supports both TCP and UDP. Mode 2 UDP connects are blocking to work
around a VPP half-open cleanup race. Mode 2 concurrent listeners are not
currently safe; see
...
In the 2026-07-21 local audit, both standard runs and the Mode 3 multi-worker
run passed (with the expected cut-through half-close skip). The Mode 2
multi-worker run failed under concurrent HTTP accept load with the signature
documented in [the accept/MQ investigation](docs/mode2_accept_mq_investigation.md).

Current top-level coverage consists of:

- 175 no-VPP tests across the public package, lifecycle registry, Mode 3
...
- 5 multi-worker stress tests, 1 sharded-accept scaling test, 1 Mode 2
  ownership invariant test, 1 Mode 2 UDP ListenPacket test, plus 1 Mode 2
...
The integration suite covers TCP and connected UDP in both modes on
...
|-- docs/architecture.md         design rationale and layer overview
|-- docs/vclnet_deep_dive.md     VPP/VCL/VLS internals and Frida comparison
|-- docs/frida_goroutine_tracking_analysis.md
|                                Frida vs VLS thread ownership analysis
...
|-- docs/mode2_accept_mq_investigation.md
|                                current Mode 2 accept/MQ failure analysis
...
- Mode 2 concurrent listeners can currently crash the VCL application during
  accept and leave an app-to-VPP control-ring hole. Mode 2 is not a production
  server path until the linked P0 is closed.
- The current source requires a `libvppcom.so` that exports
  `vls_unregister_vcl_worker`; stock VPP compatibility is pending.
