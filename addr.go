...
func ipPortFromInfo(info vclpoll.AddrInfo) (net.IP, int) {
	if info.IsV4 {
		return net.IP(info.IP[:4]), int(info.Port)
	}
	return net.IP(info.IP[:16]), int(info.Port)
}

func addrFromInfo(info vclpoll.AddrInfo) *net.TCPAddr {
	ip, port := ipPortFromInfo(info)
