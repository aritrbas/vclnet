...
// finishConnect waits for a non-blocking connect to complete (unless
// immediate is true) and checks for context expiry and shutdown. On any
// error it closes the vlsh before returning.
func finishConnect(vlsh vclpoll.VLSH, immediate bool, ctx context.Context) error {
	if !immediate {
		if err := connectWait(vlsh, ctx); err != nil {
...
func (d *Dialer) dialSingleTCP(ctx context.Context, network, address string, addr *net.TCPAddr) (net.Conn, error) {
	...
	if err := finishConnect(vlsh, immediate, ctx); err != nil {
		return nil, opError("dial", network, address, err)
	}
...
// connectErrorPollInterval controls how often the background goroutine in
// connectWait polls vppcom_session_get_error. Frequent enough to deliver
// sub-second connect failures; infrequent enough to avoid burning CPU on
// long-lived handshakes (native TLS can take hundreds of ms).
const connectErrorPollInterval = 50 * time.Millisecond

// connectWait blocks until the non-blocking connect on vlsh completes or the
// context expires. It returns nil on success and a non-nil error on failure.
//
// Two layers ensure fast failure detection even when VPP's epoll path drops
// the SESSION_CTRL_EVT_CONNECTED event (observed with app-scope-local on
// loopback):
//
//  1. A background goroutine periodically calls SessionConnectError. When VPP
//     sets the session's vpp_error field (via vcl_session_connected_handler),
//     the goroutine detects it and calls WakeVLSH to unblock the main waiter.
//  2. If PollWaitContext returns false (context expired / shutdown), a final
//     SessionConnectError check catches errors that VPP recorded but never
//     delivered via epoll.
func connectWait(vlsh vclpoll.VLSH, ctx context.Context) error {
	...
	go func() {
		ticker := time.NewTicker(connectErrorPollInterval)
		defer ticker.Stop()
		for {
			select {
			case <-pollDone:
