...
- **Alternative signal.** Instead of waiting for the CONNECTED event,
  ...
  DETACHED sessions VPP forgot to signal). Not preferred — masks the
  underlying VPP bug and burns CPU — but a fallback if the upstream fix
  is far.
...
- **Source code**
  ...
  - `internal/vclpoll/mode2.go` — Mode 2 dispatcher method routing
    through `sessionCall`.
  - `dialer.go` — union event mask constant `connectReadyEvents` and the
    post-EPOLLOUT `SessionConnectError` check in `dialSingleTCP` and
    `dialUDP`.
  - `tls.go` — same check inside `DialTLSContext`.
- **Tests**
  - `integration_test.go`:
    - `TestTCPDialConnectionRefused` (new)
    - `TestTCPDialTLSConnectionRefused` (new)
- **Documentation**
  - `summary.md` — pending-work entry + known-limitation #12.
...
- Test times out at the context deadline (3–15 s), test logs the
