...
Implementation status was rechecked on 2026-07-21. The current vclnet Mode 2
teardown uses a local VPP extension, `vls_unregister_vcl_worker`, which clears
the pthread destructor key before running the normal VLS cleanup sequence.
That preserves the pthread lifecycle contract for lifetime-pinned Go workers,
but the API must still be upstreamed or carried as a pinned VPP patch. Current
runtime limitations and pending work remain in
[../summary.md](../summary.md#3-pending-work).

The repository supports both VLS Mode 3 (default, shared worker with per-call
pinning and one lifetime-pinned VLS epoll poller) and Mode 2 (opt-in,
session-affine worker pool with per-worker epoll and sharded listeners). The
goroutine/ownership conclusions in this document are independent of the
current Mode 2 accept/MQ P0.
...
The standard VLS contract relies on that pthread destructor. vclnet's
lifetime-pinned Mode 2 workers need deterministic cleanup even though Go does
not promise the locked M/pthread will exit when the worker goroutine ends. The
local `vls_unregister_vcl_worker` API is the symmetric explicit path: on the
owning pthread it clears the destructor key and then executes `vls_mt_del`,
including `vppcom_worker_unregister`. This avoids both early raw unregister
and a later duplicate destructor call. It is a language-neutral lifecycle API,
not goroutine tracking.
...
state. For mode 2, each owner remains on a lifetime-pinned pthread and
operations route by session affinity. That pool changes VLS ownership; it
does not need to identify the calling G.
...
| CGo session-affine worker pool, mode 2 | Yes | Yes | Per-owner poller | High | Import/API change |
