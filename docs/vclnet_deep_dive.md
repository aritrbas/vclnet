...
Current qualification is narrower than the implemented API: Mode 3 is the
validated default, while the 2026-07-21 four-worker Mode 2 run still exposes a
P0 concurrent-accept crash. The current source also links to a worker-teardown
API supplied by a local VPP review patch. See
[§15.2](#152-mode-2-accepted-session-ordering-and-mq-corruption-unresolved)
and [../summary.md](../summary.md#3-pending-work).
...
`vls_mt_del` runs when an M dies, decrements `vls_mt_n_threads`, releases held
locks, and (in mode 2) calls `vppcom_worker_unregister()` to delete the VCL
worker. That is the standard VLS contract.

vclnet's lifetime-pinned Mode 2 workers cannot depend on pthread exit for
deterministic shutdown: Go may park or reuse the locked M after the goroutine
finishes. The current adjacent VPP review build therefore exposes
`vls_unregister_vcl_worker`. Called on the owning pthread, it clears the
pthread destructor key first and then invokes the normal `vls_mt_del` cleanup.
This prevents a later destructor from unregistering the same worker twice. It
is an explicit lifecycle extension, not a `vls_mt_del` guard and not a
goroutine tracker.
...
The current bridge also directly references `vls_unregister_vcl_worker`.
pkg-config does not verify that export. The adjacent VPP review commit
`032b24d04` supplies it; a stock library without it fails at link time.
...
The ownership routing above is implemented and unit-tested, but the sharded
listener path still has an independent P0 accepted-session ordering failure
under concurrent load. Ownership correctness alone does not make Mode 2
...
cleanup work, and — on each non-bootstrap worker's own pinned OS thread —
calls `vls_unregister_vcl_worker` before signalling that the worker has
retired. That API clears the pthread destructor key before executing the normal
VLS cleanup and `vppcom_worker_unregister`. Worker 0 then destroys the app and
is parked afterward because its pthread VLS destructor is unsafe once global
VLS state has been destroyed. Reinitialization is not supported. This explicit
API is currently a required downstream VPP patch.
...
Both TCP and UDP sessions are supported. Mode 2 UDP connects block on the
worker thread until fully established to avoid a VPP half-open cleanup race.
...
This is the ownership model used to obtain Mode 2 parallelism without VLS
session migration. Listeners use per-worker sharding (`SO_REUSEPORT`) with
per-worker accept loops fanning into a shared channel.
`test/run_multiworker.sh --mode 2`
...
ownership preflight stayed on the expected VCL worker. The full four-worker
Mode 2 suite currently fails under concurrent HTTP accept load even when the
dedicated scaling test later passes. Mode 3 remains the default while the
accept P0, soak, and performance rollout gates remain open.

---

## 15. Known Bugs, Workarounds, and VPP Cleanup Races

### 15.1 Half-open UDP session cleanup race (VPP upstream bug, worked around)

The pinned VPP 26.10 build has a race in half-open session cleanup. When a
Mode 2 UDP session is closed while its connect is still in the `CONNECTING`
state, VPP's deferred cleanup RPC (`session_half_open_cleanup_notify_rpc`)
can access a freed session pool slot, causing an assertion failure:

```text
session_half_open_cleanup_notify_rpc
  -> session_is_valid(session_index, thread_index)
  -> pool_is_free assertion fails (0xFA poison detected)
```

**Root cause:** The application closes a half-open UDP session. VPP frees the
session and returns the pool index. A deferred RPC dispatched on another
worker thread still references the old index, which may now be reused or
freed. The `pool_is_free` check fires because the freed session was poisoned
with `0xFA` in debug builds (`session_free`, `session.c:260`).

**Why cut-through TX was a red herring:** The initial analysis focused on
`ct_custom_tx → svm_fifo_unset_event(s->tx_fifo)` as the crash site. However,
TX while a cut-through session is in `CLOSING` or `APP_CLOSED` is expected VPP
behavior — `ct_session_close` intentionally defers cleanup so pending TX events
can still be delivered, and `ct_session_can_tx` explicitly permits those states.
The actual crash occurs earlier in the half-open cleanup path, not in the
steady-state TX drain.

**vclnet fix:** Mode 2 UDP `connectUDP4Start` and `connectUDP6Start` now block
on the worker thread until the connect completes (like Mode 3's synchronous
`mode3ConnectUDP4/6`). This ensures sessions are never returned to the caller
in a half-open state, preventing the close-while-connecting race. The
underlying VPP bug remains unfixed upstream.

**Reproducer:** `test/udp_mode2_repro.c` and `test/run_udp_mode2_repro.sh`
exercise rapid Mode 2 UDP connect/close cycles and trigger the VPP assertion
in debug builds.

**Current defenses:**

* Mode 2 UDP connects are blocking — no half-open sessions are exposed.
...
### 15.2 Mode 2 accepted-session ordering and MQ corruption (unresolved)

The latest four-worker Mode 2 run faults the VCL application inside
`vls_accept`/`vclpoll_accept_nb_full` (address `0x14`) during concurrent HTTP
accept load. VPP then reports an out-of-order element in
`svm_msg_q_free_msg`.

The immediate source-level chain is:

1. VCL cannot attach the FIFOs for an `ACCEPTED` event because its segment is
   not available in that worker's segment table.
2. The error path calls `vcl_send_session_accepted_reply` with a null session.
3. That helper reserves and locks an app-to-VPP control-ring element, then
   dereferences `s->session_index` before enqueue/unlock.
4. The application dies. Robust-mutex recovery permits a later producer to
   continue, but cannot undo the abandoned ring reservation, so VPP eventually
   observes a later element while its ring head is stuck.

The leading preceding trigger is cross-thread publication order:
`APP_ADD_SEGMENT` is staged in the creator VPP thread's FIFO while a dependent
`ACCEPTED` event can be staged in another VPP thread's FIFO. There is no global
order between those FIFOs. That hypothesis still needs a deterministic
segment-growth/order regression.

Adding a lock inside `app_wrk_send_ctrl_evt_inline` is not the fix: its flush
caller already holds the VPP-to-app MQ lock, so an inner lock can deadlock. The
required work is a null-safe VCL negative reply plus a VPP publication barrier
that guarantees add-segment before dependent session delivery. Full diagrams,
source paths, and completion criteria are in
[mode2_accept_mq_investigation.md](mode2_accept_mq_investigation.md).

### 15.3 Test infrastructure issues that were resolved
...
### 15.4 Architectural caveats (not bugs)
...
The implementation covers TCP and UDP on IPv4 and IPv6 in both modes,
including unconnected UDP via a per-peer session adapter (`ListenPacket`),
context-aware connect and accept (with async-connect completion verified
through `vppcom_session_get_error`, so refused-loopback errors surface quickly
instead of at context timeout), resettable deadlines, HTTP/1.1, layered TLS,
native VCL TLS
...
dispatcher. Mode 2 is implemented as an opt-in, session-affine worker pool
with per-worker epoll, per-worker sharded listeners, virtual handles, ownership
preflight, exact cancellation, ordered teardown, and blocking UDP connects
(to work around a VPP half-open cleanup race).

It is not accurate to describe the library as production-complete. The
2026-07-21 local matrix passed unit/race/vet/build, both standard integration
modes, and Mode 3 multi-worker, but Mode 2 multi-worker failed under concurrent
accept load. The current source also requires a locally patched
`vls_unregister_vcl_worker` export. After those P0 items, Mode 2 still needs
sustained supported-surface and teardown soak, CI history, and a reproducible
performance baseline before it can become the default. Extended native TLS
controls and a VPP version matrix remain open.
...
| `vclnet/udpconn.go` | `udpConn` (`net.Conn`) — connected UDP in both modes |
...
| `vclnet/docs/mode2_accept_mq_investigation.md` | Current Mode 2 accepted-session ordering and MQ crash analysis |
...
| `src/vcl/vcl_locked.h` | VLS public API — `vls_create`, `vls_*`, worker register/unregister APIs |
...
Mode 3 and the session-affine Mode 2 worker pool (TCP and UDP) are both
implemented. Mode 2 UDP connects are blocking to work around a VPP half-open
cleanup race. Mode 2 remains opt-in because concurrent sharded accept still has
a P0 segment-publication/error-reply crash; soak, CI history, and a
reproducible performance baseline also remain open. The current build requires
the patched `vls_unregister_vcl_worker` API. The canonical status and pending
list is [../summary.md](../summary.md).
