...
func cleanupCommand(tb interface{ Cleanup(func()) }, cmd *exec.Cmd) {
	// This cleanup owns the child process. Tests with protocol/client cleanup
	// registered later through t.Cleanup must not also defer Process.Kill:
	// function defers run first and would kill the VLS peer before the client
	// transport closes its sessions. Cleanup callbacks run LIFO, so a later
	// client cleanup completes before this process cleanup.
	tb.Cleanup(func() {
		_ = cmd.Process.Kill()
		_ = cmd.Wait()
	})
...
func TestMultiWorkerMode2NoMigration(t *testing.T) {
	...
	if got := vclpoll.Mode2UnregisterFailures(); got != 0 {
		t.Fatalf("mode-2 unregister failures=%d; worker teardown left VLS state armed", got)
	}
}
...
func TestMultiWorkerMode2ShardedAccept(t *testing.T) {
	...
	if got := vclpoll.Mode2UnregisterFailures(); got != 0 {
		t.Fatalf("mode-2 unregister failures=%d after sharded accept", got)
	}
	waitOrKill(t, cmd, stderr)
}

func TestMultiWorkerMode2UDPListenPacket(t *testing.T) {
	skipIfNotMultiWorker(t)
	if os.Getenv("VCLNET_VLS_MODE") != "2" {
		t.Skip("mode-2 UDP test only applies to VLS mode 2")
	}
	if err := vclnet.Init("vclnet-test-client"); err != nil {
		t.Fatalf("Init: %v", err)
	}
	port := reserveAPIPort()
	packetConn, err := vclnet.ListenPacket("udp4", fmt.Sprintf("127.0.0.1:%d", port))
	if err != nil {
		t.Fatalf("ListenPacket: %v", err)
	}
	defer packetConn.Close()
	t.Logf("Mode 2 UDP ListenPacket succeeded on port %d", port)
}

// TestMultiWorkerMode2RepeatedShutdown stresses the Mode 2 worker-retirement
// path (internal/vclpoll/worker.go quiesce -> rawUnregisterWorker ->
// close(w.exited), replacing the removed /proc/self/task polling) by
// spawning many independent multi-worker Mode 2 VCL apps against the same
// long-lived VPP instance, one after another. Each child process: starts N
// pinned Mode 2 workers, accepts and echoes a single connection over a
// sharded listener, then calls Shutdown (which drives every non-bootstrap
// worker through quiesce() and rawUnregisterWorker, then the bootstrap
// worker through rawAppDestroy) and exits. Because unregisterWorker resets
// the calling thread's VCL worker index before that OS thread's pthread-key
// destructor can fire (see the comment on vclpoll_worker_unregister in
// internal/vclpoll/cgo.go), no worker's destructor should ever race a
// subsequent cycle's vppcom_app_destroy or run against already-freed
// VCL/VPP state. A single VPP crash or hang anywhere in the loop fails the
// test; a live VPP after N cycles is the closest thing to empirical proof
// that no in-flight VLS call or pthread destructor is racing app teardown
// across repeated worker start/stop cycles.
//
// Deliberately uses the "shutdown" child (runShutdownSelfTest: one listener,
// one parked Accept, self-triggered Shutdown after 100ms — see line ~180),
// not "shutdown_stress". shutdown_stress drives 4 concurrent accept/dial
// goroutines against a 4-shard listener, which reproduces an unrelated,
// pre-existing race in the sharded-accept path itself (rawAcceptFull /
// vclpoll_accept_nb_full, SIGSEGV at a near-null fault address) even on
// unmodified code under concurrent multi-shard accept load — see
// summary.md's Mode 2 sharded-accept item. That bug is orthogonal to worker
// retirement and would otherwise make this test flaky for a reason unrelated
// to what it's verifying.
func TestMultiWorkerMode2RepeatedShutdown(t *testing.T) {
	skipIfNotMultiWorker(t)
	if os.Getenv("VCLNET_VLS_MODE") != "2" {
		t.Skip("mode-2 repeated-shutdown stress only applies to VLS mode 2")
	}
	...
	for i := 0; i < cycles; i++ {
		...
		waitOrKill(t, cmd, stderr)
		if t.Failed() {
			t.Fatalf("cycle %d/%d: child failed, aborting stress loop early", i+1, cycles)
		}
	}

	// A crashed or wedged VPP would make this final client fail to Init or
	// Dial; a live VPP after `cycles` full Mode 2 worker-pool start/stop
	// passes lets one more independent app attach cleanly.
	cmd, port, stderr := startServer(t, "echo", 1)
	defer cmd.Process.Kill()
	if err := vclnet.Init("vclnet-test-client-postcycle"); err != nil {
		t.Fatalf("Init after %d repeated-shutdown cycles: %v", cycles, err)
	}
	conn, err := vclnet.Dial("tcp4", fmt.Sprintf("127.0.0.1:%d", port))
	if err != nil {
		t.Fatalf("Dial after %d repeated-shutdown cycles: %v\nstderr:\n%s", cycles, err, stderr.String())
	}
	msg := []byte("vpp-still-alive-after-repeated-mode2-shutdown")
	if _, err := conn.Write(msg); err != nil {
		t.Fatalf("Write after %d repeated-shutdown cycles: %v", cycles, err)
	}
	got := make([]byte, len(msg))
	if _, err := io.ReadFull(conn, got); err != nil {
		t.Fatalf("ReadFull after %d repeated-shutdown cycles: %v", cycles, err)
	}
	if !bytes.Equal(got, msg) {
		t.Fatalf("echo mismatch after %d repeated-shutdown cycles: got %q want %q", cycles, got, msg)
	}
	_ = conn.Close()
	waitOrKill(t, cmd, stderr)
}
...
// is dropped by `vcl_ep_session_needs_evt` in some code paths).
//
// The connectWait helper works around this by polling
// vppcom_session_get_error every 50ms in a background goroutine. When the
// session error is set, WakeVLSH unblocks the main waiter. This makes the
// dial fail within ~100-500ms even when the epoll event is dropped.
//
// The test therefore requires a fast connect error (under 2s) and treats a
// context-deadline timeout as a failure — the background polling should
// always detect the error before the 3s context expires.
func TestTCPDialConnectionRefused(t *testing.T) {
	...
	if errors.Is(err, context.DeadlineExceeded) || vclnet.IsTimeout(err) {
		t.Fatalf("Dial fell back to context timeout after %v — connect-error polling failed to detect the error; err=%v",
			elapsed, err)
	}
	if elapsed > 2*time.Second {
		t.Errorf("connect error took %v (expected < 2s); background polling may not be working", elapsed)
	}
	t.Logf("Dial surfaced connect error after %v: %v", elapsed, err)
}

// TestTCPDialTLSConnectionRefused mirrors the plain-TCP refused case for
// the native VCL TLS path (DialTLSContext). The same background polling
// workaround applies, so the test requires a fast connect error.
func TestTCPDialTLSConnectionRefused(t *testing.T) {
	...
	if errors.Is(err, context.DeadlineExceeded) || vclnet.IsTimeout(err) {
		t.Fatalf("DialTLS fell back to context timeout after %v — connect-error polling failed to detect the error; err=%v",
			elapsed, err)
	}
	if elapsed > 2*time.Second {
		t.Errorf("connect error took %v (expected < 2s); background polling may not be working", elapsed)
	}
	t.Logf("DialTLS surfaced connect error after %v: %v", elapsed, err)
}
