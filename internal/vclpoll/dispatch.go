...
func Mode2UnregisterFailures() uint64 {
	if selected, ok := currentDispatcher().(*mode2Dispatcher); ok {
		return selected.unregisterFailures.Load()
	}
	return 0
}
...
// WakeVLSH unblocks any goroutine parked in PollWaitContext on vlsh without
// closing the session. Mode 3 delegates to pollUnregister; Mode 2 submits a
// wake to the owning worker. Used by the connect-error polling goroutine to
// unblock a waiter once VPP has set a session error that epoll missed.
func WakeVLSH(vlsh VLSH) {
	switch d := currentDispatcher().(type) {
	case mode3Dispatcher:
		pollUnregister(vlsh)
	case *mode2Dispatcher:
		d.wakeVLSH(vlsh)
	}
}
