...
type mode2Dispatcher struct {
	...
	unregisterFailures  atomic.Uint64
	stopOnce            sync.Once
...
func (d *mode2Dispatcher) stop() {
	d.stopOnce.Do(func() {
		...
		for _, w := range d.workers {
			...
			// close(w.exited) happens after rawUnregisterWorker() returns
			// on w's own pinned OS thread (worker.quiesce).
			// rawUnregisterWorker calls vls_unregister_vcl_worker
			// which clears the pthread-key, so vls_mt_del is a
			// guaranteed no-op when the thread eventually exits.
			<-w.exited
		}
	})
}
...
func (d *mode2Dispatcher) acceptFullContext(listener VLSH, done <-chan struct{}) (VLSH, AddrInfo, error) {
	...
	for {
		...
		if !d.pollWaitContext(listener, epollIn, done) {
			return invalidVLSH, AddrInfo{}, d.waitInterruptedError(listener, done)
		}
	}
}
...
func (d *mode2Dispatcher) wakeVLSH(handle VLSH) {
	ref, err := d.lookup(handle)
	if err != nil {
		return
	}
	_, _ = d.submit(ref.worker, func(w *worker) (any, error) {
		w.removeWaiters(handle, ref.raw)
		return nil, nil
	})
}
...
func (d *mode2Dispatcher) bindUDP4(ip [4]byte, port uint16) (VLSH, error) {
	return d.create(func() (VLSH, error) { return mode3BindUDP4(ip, port) })
}

func (d *mode2Dispatcher) bindUDP6(ip [16]byte, port uint16) (VLSH, error) {
	return d.create(func() (VLSH, error) { return mode3BindUDP6(ip, port) })
}

// connectUDP4Start blocks on the worker thread until the UDP connect completes
// or the internal timeout (10s) expires. Always returns immediate=true on
// success, so the caller never receives a half-open session handle. Closing
// a half-open Mode 2 UDP session triggers a VPP crash in
// session_half_open_cleanup_notify_rpc (stale pool index across RPC).
func (d *mode2Dispatcher) connectUDP4Start(ip [4]byte, port uint16) (VLSH, bool, error) {
	return d.createConnect(func() (VLSH, bool, error) {
		vlsh, err := mode3ConnectUDP4(ip, port)
		if err != nil {
			return invalidVLSH, false, err
		}
...
func (d *mode2Dispatcher) connectUDP6Start(ip [16]byte, port uint16) (VLSH, bool, error) {
	return d.createConnect(func() (VLSH, bool, error) {
		vlsh, err := mode3ConnectUDP6(ip, port)
		if err != nil {
			return invalidVLSH, false, err
		}
		return vlsh, true, nil
...
func (d *mode2Dispatcher) sendToContext(handle VLSH, p []byte, addr AddrInfo, done <-chan struct{}) (int, error) {
	for {
		value, err := d.sessionCall(handle, func(_ *worker, raw VLSH) (any, error) {
			n, err := rawSendTo(raw, p, addr)
			return n, err
		})
		if err == nil {
			return value.(int), nil
...
func (d *mode2Dispatcher) recvFromContext(handle VLSH, p []byte, done <-chan struct{}) (int, AddrInfo, error) {
	for {
		value, err := d.sessionCall(handle, func(_ *worker, raw VLSH) (any, error) {
			n, addr, err := rawRecvFrom(raw, p)
			if err != nil {
				return nil, err
			}
			return recvFromReply{n: n, addr: addr}, nil
		})
		if err == nil {
			reply := value.(recvFromReply)
			return reply.n, reply.addr, nil
		}
		if !isAgainError(err) {
			return 0, AddrInfo{}, err
		}
		if !d.pollWaitContext(handle, epollIn, done) {
			return 0, AddrInfo{}, d.waitInterruptedError(handle, done)
		}
	}
}
