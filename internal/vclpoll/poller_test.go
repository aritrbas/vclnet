...
func newTestPoller() (*poller, *[]epollCall, *[]epollCall, *[]VLSH) {
	...
	p := &poller{
		...
		unregCh:  make(chan unregisterRequest, 16),
...
func TestPollerUnregisterWakesEveryWaiter(t *testing.T) {
	...
	p.unregCh <- unregisterRequest{vlsh: 11, done: done}
	...
	if !isClosed(done) {
		t.Fatal("session unregister was not acknowledged")
	}
}

func TestPollUnregisterWaitsForPollerAcknowledgement(t *testing.T) {
	p, _, _, _ := newTestPoller()
	p.running.Store(true)

	previous := defaultPoller
	defaultPoller = p
	t.Cleanup(func() { defaultPoller = previous })
	...
	case <-time.After(time.Second):
		t.Fatal("unregister request was not submitted")
	}

	select {
	case <-returned:
		t.Fatal("pollUnregister returned before the poller acknowledged removal")
	default:
	}
	...
	case <-returned:
	case <-time.After(time.Second):
		t.Fatal("pollUnregister did not return after poller acknowledgement")
	}
}
...
func TestPollUnregisterBeforeStart(t *testing.T) {
	p := &poller{
		...
		unregCh:  make(chan unregisterRequest, 1),
		stopCh:   make(chan struct{}),
		stopped:  make(chan struct{}),
		waiters:  make(map[VLSH]*waitSet),
		started:  make(chan struct{}),
