...
// closeShards releases every already-created shard listener, routing each
// close through its owning worker since raw VLS handles are worker-local.
// If a worker is stopping, submit fails with ErrClosed and the raw handle is
// leaked in that worker's VLS state until process/app teardown reclaims it;
// closeShards still reports that failure to the caller instead of discarding
// it silently.
func closeShards(d *mode2Dispatcher, shards []listenerShard) error {
	var errs []error
	for _, s := range shards {
		if _, err := d.submit(s.worker, func(_ *worker) (any, error) {
			return nil, rawClose(s.raw)
		}); err != nil {
			errs = append(errs, fmt.Errorf("worker %d: %w", s.worker.id, err))
		}
	}
...
func (d *mode2Dispatcher) newShardedListener(createFn func() (VLSH, error)) (VLSH, error) {
	...
	for _, w := range d.workers {
		...
		if err != nil {
			if cleanupErr := closeShards(d, shards); cleanupErr != nil {
				err = fmt.Errorf("%w (cleanup of %d already-created shard(s) also failed: %v)", err, len(shards), cleanupErr)
			}
