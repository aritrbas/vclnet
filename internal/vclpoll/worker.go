...
type worker struct {
	...
	// unregisterWorker defaults to rawUnregisterWorker; tests override it to
	// verify quiesce() calls it, without a live VPP.
	unregisterWorker func() error
}
...
func newWorker(id int, bootstrap bool, d *mode2Dispatcher, start <-chan struct{}) *worker {
	return &worker{
		...
		unregisterWorker: rawUnregisterWorker,
	}
}
			...
			if !w.bootstrap {
				if w.unregisterWorker != nil {
					if err := w.unregisterWorker(); err != nil {
						w.dispatcher.unregisterFailures.Add(1)
					}
				}
				close(w.exited)
				return
