...
func (pc *packetConn) peerReadLoop(ps *peerSession) {
	...
	for {
		...
		// A zero-length datagram is a valid UDP payload, not an EOF signal;
		// unlike TCP, the peer session stays open for further datagrams.
		data := make([]byte, n)
		copy(data, buf[:n])
...
// removePeer closes the peer's VLS session and removes it from pc.peers so
// a subsequent WriteTo does not attempt to use the now-dead session. It is
// only called on the peer's read-error exit path; Close(pc) tears down all
// remaining peers itself.
func (pc *packetConn) removePeer(ps *peerSession) {
	key := addrToKey(ps.addr)
	pc.peerMu.Lock()
	if pc.peers[key] == ps {
		delete(pc.peers, key)
	}
...
func (pc *packetConn) WriteTo(p []byte, addr net.Addr) (int, error) {
	...
	for {
		...
		if retry, mapped := mapIOError(err); retry {
			...
			return 0, opErrorAddr("write", pc.localAddr, mapped)
		}
		return n, nil
	}
}
...
func (pc *packetConn) SetDeadline(t time.Time) error {
	if err := closedStateError(&pc.closed); err != nil {
		return opErrorAddr("set", pc.localAddr, err)
	}
